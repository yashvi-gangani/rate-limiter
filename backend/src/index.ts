...
async function main() {
  ...
  if (!mongoConnected) {
    console.log("[gateway] MONGO_URL not set/unreachable — using in-memory API keys + tiers");
  }
  await initTierCache();
  ...
  if (config.redisUrl) {
    ...
    console.log("[gateway] REDIS_URL not set — using in-memory store (single instance only)");
  }
  ...
  app.listen(config.port, () => {
    ...
    console.log(`[gateway] demo endpoints:   http://localhost:${config.port}/api/demo/ping`);
    console.log(`[gateway] metrics:          http://localhost:${config.port}/metrics`);
    console.log(`[gateway] dashboard stats:  http://localhost:${config.port}/admin/stats`);
  });
}
